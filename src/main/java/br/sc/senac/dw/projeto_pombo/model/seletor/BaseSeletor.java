...
public abstract class BaseSeletor {
	...
	public static void aplicarFiltroIntervalo(Root root, 
			CriteriaBuilder pc, List<Predicate> predicates,
			Integer valorMinimo, Integer valorMaximo, String nomeAtributo) {
		  if(valorMinimo != null && valorMaximo != null) {
          	//WHERE atributo BETWEEN min AND max
          	predicates.add(pc.between(root.get(nomeAtributo), valorMinimo, valorMaximo));
          } else if(valorMinimo != null) {
          	//WHERE atributo >= min
          	predicates.add(pc.greaterThanOrEqualTo(root.get(nomeAtributo), valorMinimo));
          } else if(valorMaximo != null) {
          	//WHERE atributo <= max
          	predicates.add(pc.lessThanOrEqualTo(root.get(nomeAtributo), valorMaximo));
          }
	}
